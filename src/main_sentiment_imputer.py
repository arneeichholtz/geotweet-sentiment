...
def imputer(file, args, imputation_method):
    ...
    df.to_csv(
        os.path.join(args.output_path, '{}_sentiment_{}.csv'.format(imputation_method, date_name)), sep='\t', index=False
    )
...
if __name__ == '__main__':
    ...
    if len(args.dict_methods)>0:
        from src.utils.dict_sentiment_imputer import parallel_imputation
    if len(args.emb_methods):
        from src.utils.emb_sentiment_imputer import embedding_imputation
